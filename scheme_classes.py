...
class Frame:
    ...
    def define(self, symbol, value):
        ...
        # BEGIN PROBLEM 1
        self.bindings[symbol] = value  # Bind symbol to value in the current frame
        # END PROBLEM 1
    ...
    def lookup(self, symbol):
        ...
        if symbol in self.bindings:
            return self.bindings[symbol]  # If symbol is bound in the current frame, return its value
        elif self.parent is not None:
            return self.parent.lookup(symbol)  # Look up the symbol in the parent frame
        else:
            raise SchemeError('unknown identifier: {0}'.format(symbol))  # Symbol not found in current or parent frame
        # END PROBLEM 1
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 8
        new_frame = Frame(self)  # Create a new Frame instance with self as the parent

        while formals is not nil and vals is not nil:
            new_frame.define(formals.first, vals.first)  # Bind each formal parameter to its corresponding argument value
